...
//#endif
				using (var db = new kipadminEntities())
				{
					...
					else
					{
						...
						if (aansluiting == null || aansluiting.REKENING.DOORGEBOE != "J")
						{
						...
						var toeTeKennen =
							db.FunctieSet.Where(Utility.BuildContainsExpression<Functie, int>(f => f.id, nationaalBepaaldeFuncties.Cast<int>()));
