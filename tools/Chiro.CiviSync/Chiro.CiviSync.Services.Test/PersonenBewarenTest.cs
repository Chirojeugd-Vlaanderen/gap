...
{
    [TestFixture]
    public class PersonenBewarenTest: SyncTest
    {
        ...
        public void ContactTypeBijGematchtLid()
        {
            ...
            using (IDiContainer factory = TestHelper.IocOpzetten(new DateTime(2015, 02, 05),
                out civiApiMock, out updateHelperMock))
            {
                ...
                civiApiMock.Setup(src => src.ContactSave(It.IsAny<string>(), It.IsAny<string>(),
                        It.Is<ContactRequest>(r => r.ContactType == ContactType.Individual)))
                    .Returns(
                        (string key1, string key2, ContactRequest r) => SomeSaveResult(r))
                    .Verifiable();
        ...
        public void CorrectGeslachtBijTeMatchenLid()
        {
                ...
                out updateHelperMock))
            {
                ...
                civiApiMock.Setup(
                    ...
                    .Returns(
                        new ApiResultValues<Contact> {Count = 1, IsError = 0, Values = new[] {persoon}})
                    .Verifiable();
                ...
                civiApiMock.Setup(
                        src =>
                            src.ContactGet(It.IsAny<string>(), It.IsAny<string>(),
                                It.Is<ContactRequest>(r => r.Gender == persoon.Gender &&
                                                           r.ContactType == ContactType.Individual)))
                    .Returns(
                        new ApiResultValues<Contact> {Count = 1, IsError = 0, Values = new[] {persoon}})
                    .Verifiable();
        ...
        public void PersoonUpdatenZonderAd()
        {
                ...
                out updateHelperMock))
            {
                ...
                civiApiMock.Setup(
                    ...
                    .Returns(TestHelper.Map<Contact, ApiResultValues<Contact>>(persoon));
                civiApiMock.Setup(
                        src =>
                            src.ContactSave(It.IsAny<string>(), It.IsAny<string>(),
                                It.Is<ContactRequest>(r => r.ExternalIdentifier == persoon.ExternalIdentifier)))
        ...
        private ApiResultValues<Contact> SomeSaveResult(ContactRequest contactRequest)
        {
            return new ApiResultValues<Contact>
            {
                ...
                Id = _nextId++,
                IsError = 0,
                Values = new Contact[] {TestHelper.Map<ContactRequest, Contact>(contactRequest)}
            };
        }
    }
}
