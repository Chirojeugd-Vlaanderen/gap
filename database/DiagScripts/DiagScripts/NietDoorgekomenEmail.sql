...
join pers.communicatievorm cv on cv.GelieerdePersoonID = gp.GelieerdePersoonID and cv.CommunicatieTypeID = 3
...
left outer join kipadmin.dbo.kipcontactinfo kci on kci.AdNr = kp.AdNr and kci.info = cv.Nummer collate latin1_general_ci_ai
left outer join 
(
select adnr, max(volgnr) as offset from kipadmin.dbo.kipcontactinfo where contacttypeid=3 group by adnr
