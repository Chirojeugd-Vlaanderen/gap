...
\noindent {\Large \bf CODING STANDARD C\#, doc. v. 0.4}
...
is.  Bijv. `IDao\_1'.
\item Ofwel worden alle \lstinline !enum!s \index{enum}\index{enum!declaratie} uit een assembly ondergebracht
...
\begin{enumerate}[resume]
\item Elk bronbestand\index{bronbestanden} begint met een blokje commentaar met daarin\opmerking{Dit hebben we niet
afgesproken, maar het lijkt me een zinvol idee.}
\begin{itemize}
\item een korte omschrijving van wat er in de code staat
\item een `copyright notice'\index{copyright}
\item een verwijzing naar de licentievoorwaarden\index{licentievoorwaarden} (?)
\item contactinfo van de verantwoordelijke voor het bestand
\end{itemize}
\item Groepeer de imports van namespaces bovenaan\index{namespaces}
\begin{itemize}
...
\item Uitzondering: \'e\'en spatie tussen \lstinline !if!\index{if}, \lstinline !for!\index{for},
...
\item Als je een bugfix\index{bugfix} implementeerde, en het is niet duidelijk waarom je
...
\end{itemize}
\item Indien mogelijk: voorzie tutorials\index{tutorials} en voorbeeldcode\index{voorbeelden} op de wiki.
...
\item Underscores\index{underscores} zijn enkel toegelaten als prefix van een member\index{membervariabelen!namen}
variable.
...
\item Geen hoofdletters\index{hoofdletters} in het midden van letterwoorden of afkortingen
\begin{itemize}
...
gemeenschappelijk suffix\index{consistentie}, zoals \lstinline !PersonenDao!, \lstinline
!GroepenDao!, \ldots
\end{enumerate}
...
als de property, maar met een kleine letter en geprefixt met een underscore.
\item Geen type-informatie\index{type-informatie} in propertynamen.
...
\item Namen van \lstinline !const!s\index{const} zijn volledig in hoofdletters.
...
\item Gebruik voor declaraties waar mogelijk interfaces\index{interfaces}, zelfs voor lokale variabelen\index{lokale variabelen!declaratie} of
...
\lstinline !int!\index{int} of \lstinline !string!\index{string}.
...
\item Vermijd \lstinline !enums! te wijzigen\index{enum!wijzigen} als ze ook gebruikt worden in code die je
...
assigneren\index{bool}; vergelijk niet met `\lstinline !true!' of `\lstinline !false!'.
...
\item Gebruik `\lstinline !String.Compare!'\index{String.Compare} om strings te vergelijken.
...
\item Gebruik `\lstinline !Equals()!' om strings te
vergelijken.\footnote{\lstinline !Equals()! heeft overloads voor
...
(voor read-only access) en \lstinline !IList<T>!\index{IList}.
...
overloads en in gelijkaardige methods.\index{consistentie}
...
\begin{enumerate}[resume]
\item Serivces zijn beveiligd.\index{beveiliging}
