...
<table class="overzicht">
	...
	foreach (LidOverzicht lidOverzicht in ViewData.Model.LidInfoLijst){
		...
		<%}%>
		<td>
			<input type="checkbox" name="SelectieGelieerdePersoonIDs" 
		...
		<td><%= lidOverzicht.Type == LidType.Kind ? "Lid" : "Leiding" %></td>
		<td><%=Html.PersoonsLink(lidOverzicht.GelieerdePersoonID, lidOverzicht.VoorNaam, lidOverzicht.Naam)%></td>
		<td class="right">
		...
		<td><%=Html.Telefoon(lidOverzicht.TelefoonNummer) %> </td>
		<td><a href='mailto:<%=lidOverzicht.Email %>'><%=lidOverzicht.Email %></a></td>
	</tr>
	<% } %>
</table>
