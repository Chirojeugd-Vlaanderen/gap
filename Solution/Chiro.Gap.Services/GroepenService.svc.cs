...
namespace Chiro.Gap.Services
{
    ...
    public class GroepenService : BaseService, IGroepenService, IDisposable
    {
        ...
        public IEnumerable<LedenProbleemInfo> LedenControleren(int groepId)
        {
            ...
            var aantalLedenMetVerdachteEmail = (from ld in _ledenRepo.Select("GelieerdePersoon.Communicatie")
                                                where ld.GroepsWerkJaar.ID == groepsWerkJaar.ID
                                                    && ld.GelieerdePersoon.Communicatie.Any(cmm => cmm.IsVerdacht == true)
                                                    && ld.UitschrijfDatum == null
                                                select ld).Count();

            if (aantalLedenMetVerdachteEmail > 0)
            {
                resultaat.Add(new LedenProbleemInfo
                {
                    Probleem = LidProbleem.EmailIsVerdacht,
                    Aantal = aantalLedenMetVerdachteEmail
                });
            }
